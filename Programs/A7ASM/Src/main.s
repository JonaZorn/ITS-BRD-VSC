...
; Tick-Konstanten: Hardware-Timer läuft mit 100.000 Ticks/Sekunde (1 Tick = 10µs)
TICKS_10MIN     	EQU 60000000                               ; Ticks für 10 Minuten
TICKS_1MIN      	EQU 6000000                                ; Ticks für 1 Minute
TICKS_10SEC     	EQU 1000000                                ; Ticks für 10 Sekunden
TICKS_1SEC      	EQU 100000                                 ; Ticks für 1 Sekunde
TICKS_10CENT		EQU	10000                                  ; Ticks für 10 Hundertstelsekunden
TICKS_1CENT			EQU 1000                                   ; Ticks für 1 Hundertstelsekunde

    EXTERN initITSboard                                    ; Initialisiert das ITS-Board (externe Funktion)
    EXTERN GUI_init                                        ; Initialisiert die grafische Oberfläche
	EXTERN TP_Init                                         ; Initialisiert den Touchscreen
...
run PROC
		...
		ldr     R1, =GPIO_D_CLR     ; Achtung: kleines 'r' wie oben definiert!
		strh    R0, [R1]            ; LED D9 ausschalten (kein HOLD)

		; --- 7. TASTER S6 ABFRAGEN (WECHSEL ZU HOLD) ---
		mov     R0, #6              ;
		ldr	    R1, =GPIO_F_PIN     ;
		ldrh    R1, [R1]            ;
		bl      isButtonPressed     ;
...
endif_S6_pressed
		...
		; --- 8. TASTER S5 ABFRAGEN (WECHSEL ZU INIT) ---
		mov     R0, #5              ;
		ldr	    R1, =GPIO_F_PIN     ;
		ldrh    R1, [R1]            ;
		bl      isButtonPressed     ;
...
; print_time: Gibt nur geänderte Zeichen des ZEIT-Strings auf dem TFT aus.
; Vergleicht ZEIT mit ZEIT_ALT zeichenweise; nur unterschiedliche Zeichen werden neu gezeichnet.
; Kein Parameter, kein Rückgabewert. Ändert nur das Display und ZEIT_ALT.
print_time PROC
...
do_sc		
		...
		BEQ		next_char           ; Ja? kein Neuzeichnen nötig, weiter zum nächsten Zeichen
...
isButtonPressed PROC
		...
		AND	    R2,R1,R2            ; Taster-Bit aus dem GPIO-Register isolieren
		cmp	    R2,#0               ; Ist das isolierte Bit 0? (= Taster gedrückt, da active-low)
		BEQ		isPressed           ; Ja? Springe zu isPressed
		mov	    R0,#0               ; Bit nicht 0 → Taster nicht gedrückt → R0 = 0
		B	    endPressed          ; Zum Ende springen
	
isPressed
		mov	    R0,#1               ; Taster gedrückt → R0 = 1
